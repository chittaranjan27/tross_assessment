...
class AppException(Exception):
    ...
    def __init__(self, message: str, code: str, http_status: int = status.HTTP_500_INTERNAL_SERVER_ERROR):
...
class InvalidProfileUrlError(AppException):
    def __init__(self, message: str = "The provided URL is not a valid LinkedIn profile URL."):
        super().__init__(message, "INVALID_PROFILE_URL", status.HTTP_400_BAD_REQUEST)

class ProfileNotFoundError(AppException):
    def __init__(self, message: str = "The requested profile could not be found or is unavailable."):
        super().__init__(message, "PROFILE_NOT_FOUND", status.HTTP_404_NOT_FOUND)

class UpstreamAuthenticationError(AppException):
    def __init__(self, message: str = "Failed to authenticate with the upstream provider."):
...
class UpstreamResponseParseError(AppException):
    def __init__(self, message: str = "Failed to parse the response from the upstream provider."):
